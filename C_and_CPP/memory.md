...
## new operator
The new operator requests for the allocation of the block of memory of the given size of type on the free storage. If sufficient memory is available, a new operator initializes the memory to the default value according to its type and returns the address to this newly allocated memory.
